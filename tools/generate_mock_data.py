...
parser = argparse.ArgumentParser(
    description="Generate mock healthcare data with specified random seed and data paths"
)
parser.add_argument(
    "--seed",
    type=int,
...
parser.add_argument(
    "--input_data_path",  # Renamed from --data_path for clarity
    type=str,
    default="input_data",  # Set a default value, but allow override
    help="Path to the directory containing input data files (default: input_data)",
)
parser.add_argument(
    "--output_data_path",  # New argument for output path
    type=str,
...
print(f"Using input data path: {args.input_data_path}")
print(f"Using output data path: {args.output_data_path}")
...
class DemographicsGenerator:
    def __init__(self, seed: int = 42, reference_date: datetime = datetime(2024, 1, 1)):
        ...
        self.indiana_demographics = {
            "age_distribution": {
                ...
                "std": 10,
                "bounds": (45, 80),
                "weights": {"45-54": 0.25, "55-64": 0.35, "65-74": 0.25, "75-80": 0.15},
            },
            "education_by_age": {
                "45-54": {
                    "High School": 0.25,
    ...
    def _generate_correlated_variables(self, n_samples: int) -> np.ndarray:
        ...
        return multivariate_normal.rvs(
            mean=[0, 0, 0], cov=self.correlation_matrix, size=n_samples
        )
    ...
    def _generate_education_level(self, age: float, random_factor: float) -> str:
        ...
        adjusted_probs = {
            k: v * (1 + random_factor * 0.2) for k, v in probabilities.items()
        }
        total = sum(adjusted_probs.values())
        adjusted_probs = {k: v / total for k, v in adjusted_probs.items()}

        return np.random.choice(
            list(adjusted_probs.keys()), p=list(adjusted_probs.values())
        )

    def _generate_income(
        self, age: float, education: str, gender: str, random_factor: float
    ) -> float:
        base_income = self.indiana_demographics["income_parameters"]["base_values"][
            education
        ]
        age_category = self._get_age_category(age)
        age_factor = self.indiana_demographics["income_parameters"]["age_adjustment"][
            age_category
        ]
        gender_factor = self.indiana_demographics["income_parameters"]["gender_factor"][
            gender
        ]
    ...
    def generate_demographics(
        self, n_patients: int, rucc_codes: List[int]
    ) -> pd.DataFrame:
        ...
        for i in range(n_patients):
            ...
            income = self._generate_income(
                age, education, gender, correlated_vars[i, 2]
            )
...
class MedicalDataGenerator:
    def __init__(self, seed: int = 42, reference_date: datetime = datetime(2024, 1, 1)):
        ...
        self.base_params = {
            "encounter": {
                ...
                "age_weights": {"45-60": 1.0, "60-75": 1.3, "75+": 1.6},
                "season_weights": {
                    1: 1.3,
                    2: 1.3,
                    12: 1.3,  # winter
            ...
            "diagnosis": {"age_risk_factors": {"45-60": 1.0, "60-75": 1.3, "75+": 1.6}},
        }
    ...
    def _calculate_travel_time_effect(self, travel_time: float) -> float:
        for threshold, weight in sorted(
            self.base_params["encounter"]["time_effects"].items()
        ):
            if travel_time <= threshold:
                return weight
        return self.base_params["encounter"]["time_effects"][float("inf")]

    def _calculate_monthly_lambda(
        self, patient: pd.Series, date: datetime, travel_time: float
    ) -> float:
    ...
    def generate_encounters(self, demographics_df, start_date, end_date, conn):
        ...
        for _, patient in demographics_df.iterrows():
            ...
            while current_date <= end_date:
                monthly_lambda = self._calculate_monthly_lambda(
                    patient, current_date, travel_time
                )
                ...
                if n_visits > 0:
                    ...
                    visit_dates = sorted(
                        [
                            current_date
                            + timedelta(days=np.random.randint(0, days_in_month))
                        ...
                        patient_encounters.append(
                            {
                                ...
                                "end_date": (
                                    visit_date + timedelta(days=duration)
                                ).strftime("%Y-%m-%d"),
    ...
    def generate_diagnoses(
        self, encounters_df: pd.DataFrame, demographics_df: pd.DataFrame
    ) -> pd.DataFrame:
        diagnoses = []
        patient_diagnoses = {}
        ...
        for _, encounter in encounters_df.iterrows():
            ...
            age = (
                self.current_date - pd.to_datetime(patient["birth_date"])
            ).days / 365.25
            ...
            if np.random.random() < max_prob:
                if selected_weights:
                    procedure_code = np.random.choice(
                        list(selected_weights.keys()), p=list(selected_weights.values())
                    )
                else:
                    procedure_code = np.random.choice(
                        ["45378", "45380", "45384", "45385"], p=[0.7, 0.2, 0.05, 0.05]
                    )
...
def load_existing_data(
    conn, patient_locations, clinic_locations, travel_times, input_data_path: str
):
    ...
    cursor.executemany(
        "INSERT INTO provider (provider_id, latitude, longitude) VALUES (?, ?, ?)",
        provider_data,
    )
    ...
    for i, location in enumerate(patient_locations):
        ...
        patient_data.append(
            (patient_id, census_block, location["lat"], location["lon"])
        )

    cursor.executemany(
        "INSERT INTO geolocation (patient_id, census_block, latitude, longitude) VALUES (?, ?, ?, ?)",
        patient_data,
    )
        ...
        for j, time in enumerate(times.values()):
            ...
            if combination not in seen_combinations:
                ...
                travel_time_data.append(
                    (census_block, provider_id, "DRIVING", float(time) * 60)
                )

    cursor.executemany(
        "INSERT INTO travel_time (census_block, provider_id, travel_time_type, travel_time_minutes) VALUES (?, ?, ?, ?)",
        travel_time_data,
    )

    # Load RUCC data from CSV using the provided input_data_path
    rucc_file_path = os.path.join(input_data_path, "rucc_codes.csv")
    rucc_df = pd.read_csv(rucc_file_path)
    rucc_data = [
        (row["census_block"], row["rucc_code"]) for _, row in rucc_df.iterrows()
    ]

    cursor.executemany(
        "INSERT INTO rucc (census_block, rucc_code) VALUES (?, ?)", rucc_data
    )
...
def generate_mock_data(conn, n_patients=100, reference_date=datetime(2024, 1, 1)):
    ...
    rucc_codes = [
        row[0]
        for row in cursor.execute("SELECT rucc_code FROM rucc ORDER BY census_block")
    ]

    # generate demographics data
    demographics_generator = DemographicsGenerator(SEED, reference_date)
    demographics_df = demographics_generator.generate_demographics(
        n_patients, rucc_codes
    )
    ...
    encounters_df = medical_generator.generate_encounters(
        demographics_df, start_date, end_date, conn
    )
    ...
    diagnoses_df = medical_generator.generate_diagnoses(encounters_df, demographics_df)
    ...
    procedures_df = medical_generator.generate_procedures(
        encounters_df, demographics_df, diagnoses_df
    )
...
def ensure_fresh_database():
    ...
    if os.path.exists("healthcare.db"):
        try:
            ...
            print(
                "Could not remove existing database. Please close any programs that might be using it."
            )
            return False
    return True
...
def main():
    ...
    try:
        # Construct paths using the input_data_path argument
        patient_locations_path = os.path.join(
            args.input_data_path, "sampled_block_group_centers_100_30.json"
        )
        travel_times_path = os.path.join(
            args.input_data_path, "ExactTravelTimeDatafromAllMatrix.json"
        )

        # Load your existing data
        with open(patient_locations_path, "r") as f:
