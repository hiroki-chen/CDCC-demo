 ...
 */
export async function startAttestation(client: SecureClient): Promise<AttestationReport> {
  ...
  // 4. Verify the quote.
  const valid = await verifyQuote(Uint8Array.from(atob(response.quote), c => c.charCodeAt(0)).buffer);
  if (!valid) {
